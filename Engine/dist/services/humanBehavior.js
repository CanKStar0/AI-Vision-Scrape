...
export async function humanClick(page, selector, options = {}) {
    ...
    try {
        ...
        const clickX = clamp(Math.round(box.x + box.width / 2 + offsetX), Math.round(box.x + 2), Math.round(box.x + box.width - 2));
        const clickY = clamp(Math.round(box.y + box.height / 2 + offsetY), Math.round(box.y + 2), Math.round(box.y + box.height - 2));
...
export async function naturalScroll(page, options = {}) {
    ...
    for (let i = 0; i < maxScrolls; i++) {
        ...
        const scrollAmount = Math.round(viewportHeight * scrollDownRatio * randFloat(0.5, 1.2));
