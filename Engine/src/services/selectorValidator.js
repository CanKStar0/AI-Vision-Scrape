...
export function validateSelectors(results, dom) {
  ...
  try {
    ...
    console.error(
      "[Validator] ⚠️ Failed to parse DOM with cheerio:",
      err.message
    );
  ...
  return results.map((result) => {
    ...
    try {
      ...
      if (elements.length === 0) {
        console.log(
          `[Validator] ❌ Selector not found: "${selector}"`
        );
        return {
          ...result,
          isValidated: false,
      ...
      if (isValidated) {
        console.log(
          `[Validator] ✅ "${result.label}" — validated (${(similarity * 100).toFixed(0)}% match)`
        );
      } else {
        console.log(
          `[Validator] ❌ "${result.label}" — FAILED (${(similarity * 100).toFixed(0)}% match)`
        );
        console.log(
          `             AI value:  "${value}"`
        );
        console.log(
          `             DOM text:  "${domText.substring(0, 100)}${domText.length > 100 ? "..." : ""}"`
        );
      }
      ...
      console.log(
        `[Validator] ⚠️ Selector error for "${selector}": ${err.message}`
      );
      return {
        ...result,
        isValidated: false,
