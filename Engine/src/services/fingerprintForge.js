...
function checkRateLimit(domain) {
  ...
  if (!session) return; // First request — no limit
  ...
  if (session.requestTimestamps.length >= RATE_LIMIT.maxRequestsPerMinute) {
    ...
    throw new Error(
      `[FingerprintForge] ⛔ Rate limit exceeded for "${domain}". ` +
  ...
  if (session.lastAccess && now - session.lastAccess < RATE_LIMIT.minDelayBetweenRequests) {
    ...
    throw new Error(
      `[FingerprintForge] ⛔ Too fast for "${domain}". ` +
...
export function getProfileForSite(url) {
  ...
  if (!session) {
    ...
    console.log(
      `[FingerprintForge] 🎭 New session for "${domain}" → Profile: "${DEVICE_PROFILES[profileIndex].id}"`
    );
  } else {
    // Record the request
